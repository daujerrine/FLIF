...
#define __SUBST__
/*\
for(int k = 0; k < images[0].numPlanes(); ++k) {\
    for(int j = 0; j < images[0].rows(); ++j) {\
        for(int i = 0; i < images[0].cols(); ++i)\
            printf("%d ", images[0].get(k, j, i));\
        printf("\n");\
    }\
    printf("===\n");\
}*/
...
struct scanline_plane_decoder: public PlaneVisitor {
    ...
    const int p, fr;
    const uint32_t r;
    const ColorVal grey, minP;
    const bool alphazero, FRA;
    scanline_plane_decoder(Coder &c, Images &i, const ColorRanges *ra, 
                           Properties &prop, const GeneralPlane &al, const int pl,
                           const int f, const uint32_t row, const ColorVal g, 
                           const ColorVal m, const bool az, const bool fra) :
                           coder(c), images(i), ranges(ra), properties(prop),
                           alpha(static_cast<const alpha_t&>(al)), p(pl), fr(f), 
                           r(row), grey(g), minP(m), alphazero(az), FRA(fra) {}

    void visit(Plane<ColorVal_intern_8>   &plane) override
    {
        flif_decode_scanline_plane(plane, coder, images, ranges, alpha,
                                   properties, p, fr, r, grey, minP, alphazero, FRA);
    }
...
#endif
//    void visit(ConstantPlane &plane) override
//    {
//         flif_decode_scanline_plane(plane, coder, images, ranges, alpha, 
//                                    properties, p, fr, r, grey, minP, alphazero, FRA);
//    }
...
uint32_t issue_callback(callback_t callback, void *user_data, uint32_t quality, 
                        int64_t bytes_read, bool decode_over, 
...
bool flif_decode_scanlines_inner(IO &io, FLIF_UNUSED(Rac &rac),
                                 std::vector<Coder> &coders, Images &images,
                                 const ColorRanges *ranges, flif_options &options,
        ...
        if (ranges->min(p) < ranges->max(p)) {
            ...
            for (uint32_t r = 0; r < images[0].rows(); r++) {
                ...
                for (int fr=0; fr< (int)images.size(); fr++) {
                    ...
                    /*printf("\n[col_begin] ");
                    for(int i = 0; i < image.col_begin.size(); ++i)
                        printf("%u ", image.col_begin[i]);
                    printf("\n[col_end] ");
                    for(int i = 0; i < image.col_end.size(); ++i)
                        printf("%u ", image.col_end[i]);
                    printf("\n");*/
                    //MSG("Frame: %u Width: %u, Height: %u, numPlanes: %u Current Plane: %u Row: %u Depth: %u\n",  fr, image.cols(), image.rows(), image.numPlanes(), p, r, image.getDepth());
                                ...
                                std::vector<Transform<IO>*> &transforms, callback_t callback, void *user_data, Images &partial_images)
{
    ...
    return flif_decode_scanlines_inner<IO,Rac,Coder>(io, rac, coders, images, ranges, options,
                                                     transforms, callback, user_data, partial_images);
}
                                      ...
                                      const int endZL, const int32_t R, const int scale, std::vector<int> &zoomlevels, std::vector<Transform<IO>*> &transforms)
{
    ...
    for (int p = 0; p < ranges->numPlanes() ; ) {
        ...
        if ( p == 4 ) {
            continue;
        }; // don't interpolate FRA lookback channel
        if (ranges->min(p) >= ranges->max(p)) {
            continue;
        };
...
struct horizontal_plane_decoder: public PlaneVisitor {
    ...
    uint32_t r = 0;
    int fr = 0;
    const alpha_t *alpha = nullptr;
    const alpha_t *planeY = nullptr;
    const int predictor;
    const int invisible_predictor;
    const int p;
...
//    const bool FRA = (nump == 5);
    ...
    for (int i = 0; i < plane_zoomlevels(images[0], beginZL, endZL); i++) {
        ...
        for(int k = 0; k < images.size(); ++k) {
            printf("pz: %d images[%d]\n", i, k);
            for(int g = 0; g < images[k].numPlanes(); ++g) {
                printf("    plane: %d :", g);
                images[k].getPlane(g).printvals();
                printf("\n");
            }
        }
        if (default_order) {
...
template<typename IO, typename Rac, typename Coder>
bool flif_decode_FLIF2_pass(IO &io, Rac &rac, Images &images, 
                            const ColorRanges *ranges, 
                            std::vector<Tree> &forest, const int beginZL, 
                            const int endZL, flif_options &options, 
                            std::vector<Transform<IO>*> &transforms,
                            callback_t callback, void *user_data, 
                            Images &partial_images)
{
...
template<typename IO, typename BitChance, typename Rac>
bool flif_decode_tree(FLIF_UNUSED(IO& io),
                      Rac &rac, 
                      const ColorRanges *ranges,
                      std::vector<Tree> &forest, 
                      const flifEncoding encoding)
{
    try {
        for (int p = 0; p < ranges->numPlanes(); p++) {
            ...
            for(size_t i = 0; i < propRanges.size(); ++i) {
                printf("?? propRanges[%lu]: (%d, %d)\n", i, propRanges[i].first, 
                       propRanges[i].second);
            }
            MetaPropertySymbolCoder<BitChance, Rac> metacoder(rac, propRanges);
            if (ranges->min(p)<ranges->max(p))
                if (!metacoder.read_tree(forest[p])) {
                    return false;
                }
        }
...
//      v_printf(2,"Decoding rough data\n");
        if (!flif_decode_FLIF2_pass<IO, RacIn<IO>, 
            FinalPropertySymbolCoder<FLIFBitChancePass2, RacIn<IO>, bits> >
            (io, rac, images, ranges, forest, images[0].zooms(), roughZL+1, 
            options, transforms, callback, user_data, partial_images)) {
            printf("<><><><><>a\n");
            std::vector<int> zoomlevels(ranges->numPlanes(),roughZL);
            flif_decode_FLIF2_inner_interpol(images, ranges, 0, 0, -1, scale, 
                                             zoomlevels, transforms);
            return false;
        }
    }
    if (options.method.encoding == flifEncoding::interlaced && 
        (options.quality <= 0 || pixels_done >= pixels_todo) &&
        pixels_todo > 1) {
        v_printf(3,"Not decoding MANIAC tree (%i pixels done,"\
                   "had %i pixels to do)\n", pixels_done, pixels_todo);
        std::vector<int> zoomlevels(ranges->numPlanes(),roughZL);
        flif_decode_FLIF2_inner_interpol(images, ranges, 0, 0, -1,
                                         scale, zoomlevels, transforms);
        printf("<><><><><>b\n");
        return pixels_done >= pixels_todo;
    } else {
        printf("<><><><><>decode tree mode\n");
        v_printf(3,"Decoded header + rough data. Decoding MANIAC tree.\n");
        if (!flif_decode_tree<IO, FLIFBitChanceTree, RacIn<IO>>
            (io, rac, ranges, forest, options.method.encoding)) {
            printf("<><><><><>c\n");
            // What is the point of having this condition
            if (options.method.encoding == flifEncoding::interlaced) {
                v_printf(1,"File probably truncated in the middle of" \
                           "MANIAC tree representation. Interpolating.\n");
                std::vector<int> zoomlevels(ranges->numPlanes(),roughZL);
                flif_decode_FLIF2_inner_interpol(images, ranges, 0, 0, -1,
                                                 scale, zoomlevels, transforms);
            }
    ...
    switch(options.method.encoding) {
        case flifEncoding::nonInterlaced:
            printf("<><><><><>4\n");
            v_printf(3,"Decoding data (scanlines)\n");
            return flif_decode_scanlines_pass<IO, RacIn<IO>, 
                   FinalPropertySymbolCoder<FLIFBitChancePass2,
                   RacIn<IO>, bits> >(io, rac, images, ranges, forest, options, 
                   transforms, callback, user_data, partial_images);
            break;
        case flifEncoding::interlaced:
            printf("<><><><><>d\n");
            v_printf(3,"Decoding data (interlaced)\n");
            return flif_decode_FLIF2_pass<IO, RacIn<IO>,
                   FinalPropertySymbolCoder<FLIFBitChancePass2,
                   RacIn<IO>, bits> >(io, rac, images, ranges, forest, roughZL, 0, 
                   options, transforms, callback, user_data, partial_images);
            break;
    }
    return false;
...
//    printf("chunk: %s\n", metadata.name);
    ...
    if (strcmp(metadata.name,"iCCP")
        ...
        if (metadata.name[0] > 'Z') v_printf(1,"Warning: Encountered unknown chunk: %s\n",metadata.name);
            ...
            e_printf("Error: Encountered unknown critical chunk: %s\n",metadata.name);
            return -1;
        }
    }
...
template <typename IO>
bool flif_decode(IO& io, Images &images, callback_t callback, void *user_data, 
                 int first_callback_quality, Images &partial_images, 
                 flif_options &options, metadata_options &md, FLIF_INFO* info)
{
    ...
    else if (scale != 1 && scale != 2 && scale != 4 && scale != 8 && scale != 16 
    ...
    char buff[5];
    if (!io.gets(buff,5)) {
        e_printf("Could not read header from file: %s\n",io.getName());
        return false;
    }
    ...
    if (!strcmp(buff,"!<ar")) {
        ...
        char ar_header[61];
        while (true) {
            if (!io.gets(ar_header,61)) {
                e_printf("Archive does not contain a FLIF image\n");
                return false;
            }
            if (!strncmp(ar_header,"__image.flif/",13)) {
                if (!io.gets(buff,5)) {
                    e_printf("Corrupt archive?\n");
                    return false;
                }
                break;
            } else {
    ...
    if (c < ' ' || c > ' '+32+15+32) {
        e_printf("Invalid or unknown FLIF format byte\n");
        return false;
    }
    c -= ' ';
    int numFrames=1;
    ...
    if (c/16 == 1)      encoding = flifEncoding::nonInterlaced;
        ...
        e_printf("Invalid or unknown FLIF encoding method\n");
        return false;
    }

    options.method.encoding = encoding;
    if (encoding == flifEncoding::nonInterlaced && options.show_breakpoints) {
        e_printf("Non-interlaced FLIF file, no breakpoints to report.\n");
        return false;
    }
    if (scale != 1 && encoding==flifEncoding::nonInterlaced && !just_identify) {
        e_printf("Cannot decode non-interlaced FLIF file at lower scale!\n");
        return false;
    }
    if (quality < 100 && encoding==flifEncoding::nonInterlaced) {
        v_printf(1,"Cannot decode non-interlaced FLIF file at lower quality! Ignoring quality...\n");
    }
    int numPlanes=c%16;
    if (numPlanes < 1 || numPlanes > 4 || numPlanes == 2) {
        e_printf("Invalid FLIF header (unsupported color channels)\n");
        return false;
    }
    if (!ioget_int_8bit (io, &c))
        return false;
    if (c < '0' || c > '2')  {
        e_printf("Invalid FLIF header (unsupported color depth)\n");
        return false;
    }
...
//        v_printf(2," [%i] %i bpp (%i..%i)",p,ilog2(image.max(p)+1),image.min(p), image.max(p));
    ...
    if (just_identify) {
        ...
        if (numPlanes == 1) printf("grayscale");
        else if (numPlanes == 3) printf("RGB");
        else if (numPlanes == 4) printf("RGBA");
    ...
    if (rw < 0 || rh < 0) {
        e_printf("Negative target dimension? Really?\n");
        return false;
    }
    int target_w = rw, target_h = rh;
    if (fit) {
        if (rw <= 0 && rh <= 0) {
            e_printf("Invalid target dimensions.\n");
            return false;
        }
    ...
    if (scale != 1 && encoding==flifEncoding::nonInterlaced) {
        v_printf(1,"Cannot decode non-interlaced FLIF file at lower scale! Ignoring resize target...\n");
        scale = 1;
    }
    ...
    while (rac.read_bit()) {
        ...
        if (tnb <= tpnb) {
            e_printf("\nTransformation '%s' is invalid given the previous transformations."
                     "nCorrupt file? Or try upgrading your FLIF decoder?\n", desc.c_str());
            return false;
        }
        tpnb = tnb;
        ...
        if (desc == "Duplicate_Frame") {
            if (images.size()<2) return false;
            else trans->configure(images.size());
        }
#endif
        if (desc == "Palette_Alpha") {
            trans->configure(images[0].alpha_zero_special);
        }
    ...
    // ---------------------------------------
    for (int i=0; i<ranges->numPlanes(); i++) 
        printf("!!! %d %d\n",  ranges->min(i), ranges->max(i));
    ...
    v_printf(9,"%lu subpixels done (%i channels), %lu subpixels todo, quality target %i%%\n",
             (long unsigned) pixels_done,realnumplanes, (long unsigned) pixels_todo,
             (int) quality);
    ...
    if (images[0].palette && ranges->max(1) < 256  
        && options.keep_palette && (ranges->numPlanes() < 4 
        || ranges->min(3)==ranges->max(3))) 
        smaller_buffer = true;

    // Y plane shouldn't be constant, even if it is (because we want to 
    // avoid special-casing fast Y plane access)
    if (!smaller_buffer) for (int fr = 0; fr < numFrames; fr++)
        images[fr].undo_make_constant_plane(0);
    ...
    // (on correct input, all other planes should be constant if alpha 
    // is always zero and alpha_zero_special holds, so it doesn't really matter.
    ...
    if (mbits >10) bits=18;
    if (mbits > bits) {
        e_printf("FLIF cannot decode >16 bit per channel. How did this happen?\n");
        return false;
    }
#else
    if (mbits > bits) {
        e_printf("This FLIF cannot decode >8 bit per channel. Please compile with SUPPORT_HDR.\n");
        return false;
    }
#endif
    bool fully_decoded;
    if (bits == 10) {
        fully_decoded = flif_decode_main<10>(rac, io, images, ranges, 
                                             transform_ptrs, options, callback, 
                                             user_data, partial_images);
#ifdef SUPPORT_HDR
    } else {
        fully_decoded = flif_decode_main<18>(rac, io, images, ranges, 
                                             transform_ptrs, options, callback,
                                             user_data, partial_images);
#endif
    }
    ...
    if (numFrames==1)
        v_printf(2,"Decoded input file %s, %li bytes for %ux%u pixels (%.4fbpp)   \n",
                 io.getName(),io.ftell(), images[0].cols()/scale, images[0].rows()/scale,
                 8.0 * io.ftell()/images[0].rows()*scale*scale/images[0].cols());
    else
        v_printf(2,"Decoded input file %s, %li bytes for %i frames of %ux%u pixels (%.4fbpp)   \n",
                 io.getName(),io.ftell(), numFrames, images[0].cols()/scale, images[0].rows()/scale, 
                 8.0*io.ftell()/numFrames/images[0].rows()*scale*scale/images[0].cols());
    ...
    if (!options.crc_check) {
        ...
        if (contains_checksum) {
            ...
            if (alphazero && options.crc_check) for (Image& image : images) 
                image.make_invisible_rgb_black();
            ...
            if (checksum != checksum2) {
                v_printf(1,"\nCORRUPTION DETECTED: checksums don't match \
                            (computed: %x v/s read: %x)! (partial file?)\n\n", 
                         checksum, checksum2);
            } else {
    ...
    if (callback && progressive_qual_target > progressive_qual_shown) {
        ...
        issue_callback(callback, user_data, 10000 * pixels_done/pixels_todo, 
                       io.ftell(), true, populatePartialImages);
    }
...
template bool flif_decode(FileIO& io, Images &images, callback_t callback, 
                          void *user_data, int, Images &partial_images, 
                          flif_options &, metadata_options &, FLIF_INFO* info);
template bool flif_decode(BlobReader& io, Images &images, callback_t callback, 
                          void *user_data, int, Images &partial_images, 
                          flif_options &, metadata_options &, FLIF_INFO* info);
