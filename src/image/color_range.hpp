...
public:
    ...
    virtual void minmax(const int p, const prevPlanes &, ColorVal &minv, ColorVal &maxv) const
    {
        minv=min(p);
        maxv=max(p);
    ...
    virtual const ColorRanges* previous() const
    {
        return NULL;
    }
};
...
public:
    ...
    ColorVal min(int p) const override
    {
        ...
        assert(p<numPlanes());
        return ranges[p].first;
    }
    ColorVal max(int p) const override
    {
        if (p >= numPlanes()) return 0;
        assert(p<numPlanes());
        return ranges[p].second;
    }
};
...
public:
    ...
    {
        ranges->snap(p,pp,minv,maxv,v);
    }
    bool isStatic() const override
    {
        return ranges->isStatic();
    }
