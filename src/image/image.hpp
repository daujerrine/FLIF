...
struct FourColorVals {
    ...
    {
        return vec.m128i_i32[i];
    }
    FourColorVals VCALL operator+(FourColorVals b)
    {
        return FourColorVals(_mm_add_epi32(vec,b.vec));
    }
    FourColorVals VCALL operator-(FourColorVals b)
    {
        return FourColorVals(_mm_sub_epi32(vec,b.vec));
    }
    FourColorVals VCALL operator-()
    {
        return FourColorVals(_mm_sub_epi32(_mm_setzero_si128(),vec));
    }
    FourColorVals VCALL operator>>(int n)
    {
        return FourColorVals(_mm_srai_epi32(vec,n));
    }
...
inline FourColorVals VCALL operator-(int a, FourColorVals b)
{
    return FourColorVals(_mm_sub_epi32(_mm_set1_epi32(a),b.vec));
}
inline FourColorVals VCALL operator>(FourColorVals a, FourColorVals b)
{
    return FourColorVals(_mm_cmpgt_epi32(a.vec,b.vec));
}
struct EightColorVals {
    ...
    {
        return vec.m128i_i16[i];
    }
    short operator[](int i) const
    {
        return vec.m128i_i16[i];
    }
    EightColorVals VCALL operator+(EightColorVals b)
    {
        return EightColorVals(_mm_add_epi16(vec,b.vec));
    }
    EightColorVals VCALL operator-(EightColorVals b)
    {
        return EightColorVals(_mm_sub_epi16(vec,b.vec));
    }
    EightColorVals VCALL operator-()
    {
        return EightColorVals(_mm_sub_epi16(_mm_setzero_si128(),vec));
    }
    EightColorVals VCALL operator>>(int n)
    {
        return EightColorVals(_mm_srai_epi16(vec,n));
    }
    VCALL operator __m128i() const
    {
        return vec;
    }
};
inline EightColorVals VCALL operator-(short a, EightColorVals b)
{
    return EightColorVals(_mm_sub_epi16(_mm_set1_epi16(a),b.vec));
}
inline EightColorVals VCALL operator>(EightColorVals a, EightColorVals b)
{
    return EightColorVals(_mm_cmpgt_epi16(a.vec,b.vec));
}
...
public:
    ...
    void set_fast(FLIF_UNUSED(size_t r), FLIF_UNUSED(size_t c), FLIF_UNUSED(ColorVal x)) override
    {
        assert(x == color);
    }
...
//      v_printf(3,"%ix%i\n",width,height);
    ...
    ColorVal get(int p, size_t r, size_t c)
    {
        assert(p>=0);
        assert(p<num);
        //printf("w: plane = %u row = %u col = %u value = %d\n", plane, row, col, value);
    ...
    size_t scaledRows() const
    {
        return SCALED(height);
    }
    size_t scaledCols() const
    {
        return SCALED(width);
    }
