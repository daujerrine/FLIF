...
public:
    ...
    {
        if (images[0].alpha_zero_special) alpha_zero_special = true;
        else alpha_zero_special = false;
        ...
        for (Image& image : images) {
            for (uint32_t r=0; r<image.rows(); r++) {
                for (uint32_t c=0; c<image.cols(); c++) {
                    ...
                    if (alpha_zero_special && std::get<0>(C) == 0) {
                        std::get<1>(C) = std::get<2>(C) = std::get<3>(C) = 0;
                    }
                    ColorVal P=0;
    ...
    void save(const ColorRanges *srcRanges, RacOut<IO> &rac) const override
    {
        ...
        if (sorted) {
            ...
            for (Color c : Palette_vector) {
                ...
                pp[0]=Y;
                srcRanges->minmax(1,pp,std::get<2>(min), std::get<2>(max));
                ColorVal I=std::get<2>(c);
                coderI.write_int2(std::get<2>(min), std::get<2>(max), I);
                pp[1]=I;
                srcRanges->minmax(2,pp,std::get<3>(min), std::get<3>(max));
...
//        printf("Loading %lu colors: ", size);
        ...
        if (sorted) {
            ...
            for (unsigned int p=0; p<size; p++) {
                ...
                if (alpha_zero_special && A == 0) {
                    Palette_vector.push_back(Color(0,0,0,0));
                    continue;
                }
