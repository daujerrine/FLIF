...
public:
    ...
    ColorBucket& findBucket(const int p, const prevPlanes &pp)
    {
        ...
        if (p==1) {
            assert((pp[0]-min0)/CB0a >= 0 && (pp[0]-min0)/CB0a < (int)bucket1.size());
            return bucket1[(pp[0]-min0)/CB0a];
        }
        if (p==2) {
            assert((pp[0]-min0)/CB0b >= 0 && (pp[0]-min0)/CB0b < (int)bucket2.size());
...
protected:
    ...
    const ColorBucket& bucket(const int p, const prevPlanes &pp) const
    {
        return buckets->findBucket(p,pp);
    }
public:
    ...
    void snap(const int p, const prevPlanes &pp, ColorVal &minv, ColorVal &maxv, ColorVal &v) const override
    {
        ...
        maxv=b.max;
        if (b.min > b.max) {
            e_printf("Corruption detected!\n");    // this should only happen on malicious input files
            minv=v=min(p);
            maxv=max(p);
            return;
        }
...
//        SimpleSymbolCoder<FLIFBitChanceMeta, RacIn<IO>, 18> coder(rac);
        ...
        for (ColorBucket& b : cb->bucket1) {
            b=load_bucket(coders, srcRanges, 1, pixelL, pixelU);
            pixelL[0] += CB0a;
            pixelU[0] += CB0a;
        }
...
//        printf("\nSaving I Color Buckets\n  ");
        pixelL.push_back(cb->min0);
        pixelU.push_back(cb->min0+CB0a-1);
        for (auto& b : cb->bucket1) {
            save_bucket(b, coders, srcRanges, 1, pixelL, pixelU);
            pixelL[0] += CB0a;
            pixelU[0] += CB0a;
        }
        ...
        for (const Image& image : images)
            for (uint32_t r=0; r<image.rows(); r++) {
                for (uint32_t c=0; c<image.cols(); c++) {
                    ...
                    if (image.alpha_zero_special && p>3 && pixel[3]==0) {
                        cb->findBucket(3, pixel).addColor(0,max_per_colorbucket[3]);
                        continue;
                    }
                    cb->addColor(pixel);
                }
