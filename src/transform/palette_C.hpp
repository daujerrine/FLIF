...
protected:
    ...
    int nb_colors[4];
public:
    ColorRangesPaletteC(const ColorRanges *rangesIn, const int nb[4]) : ranges(rangesIn)
    {
        for (int i=0; i<4; i++) nb_colors[i] = nb[i];
    }
    ...
    ColorVal min(FLIF_UNUSED(int p)) const override
    {
        return 0;
    }
