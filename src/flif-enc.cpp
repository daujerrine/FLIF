...
template<typename RAC> void static write_name(RAC& rac, std::string desc)
{
    ...
    if (transforms[nb] != desc) {
        e_printf("ERROR: Invalid transform: '%s'\n",desc.c_str());
        return;
    }
...
bool flif_encode(IO& io, Images &images, const std::vector<std::string> &transDesc, flif_options &options)
{
    ...
    if (adaptive) { // images[0] is the still image to be encoded, images[1] is the saliency map for adaptive lossy
        ...
        if (numFrames != 2) {
            e_printf("Expected two input images: [IMAGE] [ADAPTIVE-MAP]\n");
            return false;
        }
    ...
    for (int p = 0; p < numPlanes; p++) {
        if (images[0].max(p) != 255) c='2';
    }
    if (c=='2') {
        for (int p = 0; p < numPlanes; p++) {
            if (images[0].max(p) != 65535) c='0';
        }
    }
    io.fputc(c);
...
//    metaCoder.write_int(0,1,0); // FLIF version: FLIF16
    ...
    if (c=='1') {
        v_printf(3,", %i channels, 8-bit",numPlanes);
    } else if (c=='2') {
        v_printf(3,", %i channels, 16-bit",numPlanes);
    }
...
#endif
    if (mbits > bits) {
        e_printf("OOPS: this FLIF only supports 8-bit RGBA (not compiled with SUPPORT_HDR)\n");
        return false;
    }
